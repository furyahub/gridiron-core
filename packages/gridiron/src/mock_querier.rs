...
/// This uses the Gridiron CustomQuerier.
...
#[derive(Clone, Default)]
pub struct GridironFactoryQuerier {
    pairs: HashMap<String, PairInfo>,
}

impl GridironFactoryQuerier {
    pub fn new(pairs: &[(&String, &PairInfo)]) -> Self {
        GridironFactoryQuerier {
...
struct DefaultQueryHandler {
    ...
    gridiron_factory_querier: GridironFactoryQuerier,
}
        ...
        match &request {
            QueryRequest::Wasm(WasmQuery::Smart {
                ...
                FactoryQueryMsg::Pair { asset_infos } => {
                    ...
                    match self.gridiron_factory_querier.pairs.get(&key) {
...
impl WasmMockQuerier {
    pub fn new(base: MockQuerier<Empty>) -> Self {
        WasmMockQuerier {
            query_handler: DefaultQueryHandler {
                base,
                gridiron_factory_querier: GridironFactoryQuerier::default(),
            },
            cw20_query_handler: CW20QueryHandler {
    ...
    // Configure the Gridiron pair
    pub fn with_gridiron_pairs(&mut self, pairs: &[(&String, &PairInfo)]) {
        self.query_handler.gridiron_factory_querier = GridironFactoryQuerier::new(pairs);
    }
