...
pub struct InstantiateMsg {
    ...
    /// Amount of GRID distributed per block among all pairs
    pub tokens_per_block: Uint128,
    /// Start block for distributing GRID
    pub start_block: Uint64,
    /// The GRID vesting contract that drips GRID rewards
...
#[cw_serde]
pub enum ExecuteMsg {
    /// Update the address of the GRID vesting contract
    /// ## Executor
    ...
    /// Set a new amount of GRID to distribute per block
    /// ## Executor
    /// Only the owner can execute this.
    SetTokensPerBlock {
        /// The new amount of GRID to distro per block
        amount: Uint128,
    },
...
pub enum ExecuteOnReply {
    ...
    /// Sets a new amount of GRID to distribute per block between all active generators
    SetTokensPerBlock {
        /// The new amount of GRID to distribute per block
        amount: Uint128,
    },
...
pub enum QueryMsg {
    ...
    /// the total pending amount of GRID and proxy rewards claimable by generator stakers (for that LP token)
    #[returns(PoolInfoResponse)]
    PoolInfo { lp_token: String },
    /// SimulateFutureReward returns the amount of GRID that will be distributed until a future block and for a specific generator
    #[returns(Uint128)]
...
pub struct UserInfo {
    ...
    pub amount: Uint128,
    /// The amount of GRID rewards a user already received or is not eligible for; used for proper reward calculation
...
pub struct PoolInfoResponse {
    /// The slice of GRID that this pool's generator gets per block
    pub alloc_point: Uint128,
    /// Amount of GRID tokens being distributed per block to this LP pool
    pub grid_tokens_per_block: Uint128,
    ...
    /// Total amount of GRID rewards already accumulated per LP token staked
    pub global_reward_index: Decimal,
    /// Pending amount of total GRID rewards which are claimable by stakers right now
    pub pending_grid_rewards: Uint128,
