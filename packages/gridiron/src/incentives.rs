...
pub enum ExecuteMsg {
    ...
    /// Set a new amount of GRID to distribute per seconds.
    /// Only the owner can execute this.
    SetTokensPerSecond {
        /// The new amount of GRID to distribute per second
        amount: Uint128,
    },
    ...
    /// Pools which contain these tokens can't be incentivized with GRID rewards.
...
pub enum QueryMsg {
    ...
    #[returns(Vec<(String, Uint128)>)]
    /// Returns the list of all pools receiving grid emissions
    ActivePools {},
}
...
pub struct Config {
    ...
    pub grid_token: AssetInfo,
    /// Total amount of GRID rewards per second
    pub grid_per_second: Uint128,
    /// Total allocation points. Must be the sum of all allocation points in all active generators
    pub total_alloc_points: Uint128,
    /// The vesting contract which distributes internal (GRID) rewards
...
pub enum RewardType {
    /// Internal rewards aka GRID emissions don't have next_update_ts field and they are paid out from Vesting contract.
    Int(AssetInfo),
