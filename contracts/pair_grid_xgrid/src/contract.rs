...
use gridiron::pair::{ReverseSimulationResponse, SimulationResponse};
use gridiron::pair_bonded::{Config, ExecuteMsg};
use gridiron::querier::{query_supply, query_token_balance};
use gridiron::staking::Cw20HookMsg as StakingCw20HookMsg;
use gridiron_pair_bonded::base::PairBonded;
use gridiron_pair_bonded::error::ContractError;
use gridiron_pair_bonded::state::CONFIG;
...
impl<'a> PairBonded<'a> for Contract<'a> {
    ...
    fn swap(
        ...
        if offer_asset.info.equal(&AssetInfo::Token {
            contract_addr: params.grid_addr.clone(),
        }) {
            messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: params.grid_addr.to_string(),
            ...
            messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: params.xgrid_addr.to_string(),
                msg: to_json_binary(&Cw20ExecuteMsg::Send {
                    contract: params.staking_addr.to_string(),
                    amount: offer_asset.amount,
    ...
    /// Simulation swap using Gridiron Staking contract.
    fn query_simulation(
        &self,
        deps: Deps,
        _env: Env,
        ...
        let total_deposit =
            query_token_balance(&deps.querier, &params.grid_addr, &params.staking_addr)?;
        let total_shares = query_supply(&deps.querier, &params.xgrid_addr)?;

        let return_amount = if offer_asset.info.equal(&AssetInfo::Token {
            contract_addr: params.grid_addr,
        }) {
    ...
    /// Reverse simulation swap using Gridiron Staking contract.
