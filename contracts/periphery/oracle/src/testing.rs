...
fn oracle_overflow() {
    ...
    let grid_asset_info = AssetInfo::Token {
        contract_addr: grid_token_contract.clone(),
    };
    let usdc_asset_info = AssetInfo::Token {
        contract_addr: usdc_token_contract.clone(),
    };
    let grid_asset = Asset {
        info: grid_asset_info.clone(),
        amount: Uint128::zero(),
    };
    ...
    let asset = vec![grid_asset.clone(), usdc_asset.clone()];

    let instantiate_msg = InstantiateMsg {
        factory_contract: factory.to_string(),
        asset_infos: vec![grid_asset_info, usdc_asset_info],
    };
